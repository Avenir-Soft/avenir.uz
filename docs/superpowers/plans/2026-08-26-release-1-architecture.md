...
**Ход исполнения (26 августа):** задачи 1–4 пройдены, результаты — в `2026-08-26-prod-before.md`. Ожидаемое отличие от плана в задаче 3: `429` приходит уже со второго частого запроса, потому что кроме окна «5 за 10 минут» действует минимальный интервал 15 секунд. Задача 5 — стоп: пуш и слияние только после явного разрешения владельца.
